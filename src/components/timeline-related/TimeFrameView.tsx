...
export const TimeFrameView = observer((props: { element: EditorElement }) => {
  ...
  const handleWheel = (event: React.WheelEvent<HTMLDivElement>) => {
    const zoomFactor = event.deltaY > 0 ? 0.9 : 1.1; // Adjust the zoom factor as needed
    store.setMaxTime(store.maxTime * zoomFactor);
  };
  ...
  return (
    ...
    >
      <div
        ...
        key={element.id}
        className={`relative width-full h-[40px] my-2 ${
          isSelected ? "border-2 border-indigo-600 bg-slate-200" : ""
        }`}
      >
        <DragableView
